...
if(isset($_POST['login-submit'])) { 
     ...
     if(empty($user) || empty($pass)) {
      	...
      	$sql1 = $conn->query("SELECT username FROM tbl_admin WHERE username = '$user'");
      	if ($sql1->num_rows > 0) {
      		$sql = $conn->query("SELECT password FROM tbl_admin WHERE username = '$user'");
            $data = $sql->fetch_assoc();
            $hash = $data['password'];
            $pass1 = password_verify($pass,$hash);
             if($pass1){
                $sesi = $conn->query("SELECT * FROM tbl_admin WHERE username='$user'");
